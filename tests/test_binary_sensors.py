...
def test_removed_threshold_sensors_not_present() -> None:
    ...
    with open(
        "/home/dmalloc/pidev/custom_components/ontario_energy_pricing/binary_sensor.py",
        "r",
    ) as f:
        code = f.read()

    # Should NOT contain these removed sensor class names
    assert "OntarioPriceBelowThresholdSensor" not in code, (
        "OntarioPriceBelowThresholdSensor should not exist"
    )
    assert "OntarioPriceAboveThresholdSensor" not in code, (
        "OntarioPriceAboveThresholdSensor should not exist"
    )

    # Should contain only the kept sensors
    assert "OntarioCheapestHoursBinarySensor" in code, (
        "OntarioCheapestHoursBinarySensor should exist"
    )
    assert "OntarioNegativePriceSensor" in code, (
        "OntarioNegativePriceSensor should exist"
    )
...
def test_threshold_values_not_hardcoded_in_binary_sensor() -> None:
    ...
    assert "5.0" not in code or "15.0" in code, (
        "Threshold 5.0 should not be hardcoded (except as part of 15.0)"
    )
    assert "10.0" not in code, "Threshold 10.0 should not be hardcoded"
    assert (
        "20.0" not in code or "20.0" in code and "price_elevated_threshold" in code
    ), "Threshold 20.0 should not be hardcoded for price"
...
def test_grid_stressed_logic() -> None:
    ...
    assert "gas_dominant or carbon_high" in code, (
        "Grid stressed should check gas_dominant OR carbon_high"
    )
    assert "renewable_low or price_trending_up" in code, (
        "Grid stressed should check renewable_low OR price_trending_up"
    )
...
def test_rolling_price_history_in_coordinator() -> None:
    ...
    with open(
        "/home/dmalloc/pidev/custom_components/ontario_energy_pricing/coordinator.py",
        "r",
    ) as f:
        code = f.read()
